...
export class Prompt {
    ...
    category;
    tags;
    constructor(id, title, instructions, template, category, tags) {
    ...
    static fromPlainObject(data) {
        return new Prompt(data.id, data.title, data.instructions, data.template, data.category, data.tags);
    }
    /**
     * Validates prompt properties according to domain rules.
     * @throws {ValidationError} if validation fails
     */
    validate(id, title, instructions, template, category, tags) {
        const errors = [];
        if (!id?.trim())
